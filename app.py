...
# Clamp step_index + handle autoplay advance BEFORE slider renders
if st.session_state.step_index > max_idx:
    st.session_state.step_index = max_idx
...
# ── Navigation callbacks (run before widgets on next rerun) ─────────────
...
def _go_next() -> None:
    ...
    if r and r.snapshots:
        mx = len(r.snapshots) - 1
        st.session_state.step_index = min(mx, st.session_state.step_index + 1)


def _go_last() -> None:
    r = st.session_state.get("trace_result")
    if r and r.snapshots:
        st.session_state.step_index = len(r.snapshots) - 1
    st.session_state.playing = False
...
st.slider("paso", 0, max_idx, key="step_index", label_visibility="collapsed")
...
with col_code:
    _code_viewer(source, snap.line_no, snap.event)
...
with col_state:
    ...
    # ── stdout ──────────────────────────────────────────────────────────
    stdout_text = result.stdout[: snap.stdout_len]
...
if st.session_state.playing and result is not None:
    if st.session_state.step_index < max_idx:
